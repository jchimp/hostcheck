#!/usr/bin/env bash
###############################################################################
# install-oauth-token-monitor.sh
#
...
INSTALL_BIN="/usr/local/bin/oauth-token-monitor.sh"
CONF_DIR="/etc/oauth-token-monitor"
CONF_FILE="${CONF_DIR}/oauth-token-monitor.conf"
STATE_DIR="/var/lib/oauth-token-monitor"
CRON_FILE="/etc/cron.d/oauth-token-monitor"

echo "==> Installing oauth-token-monitor"

# ── Copy script ──────────────────────────────────────────────────────────────
cp "${SCRIPT_DIR}/oauth-token-monitor.sh" "$INSTALL_BIN"
...
if [[ -f "$CONF_FILE" ]]; then
  ...
  cp "${SCRIPT_DIR}/oauth-token-monitor.conf" "${CONF_FILE}.new"
else
  cp "${SCRIPT_DIR}/oauth-token-monitor.conf" "$CONF_FILE"
  chmod 600 "$CONF_FILE"
...
echo "  6. Monitor log:     tail -f /var/log/oauth-token-monitor.log"
echo ""
